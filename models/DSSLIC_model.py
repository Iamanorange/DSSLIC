...
class DSSLICModel(BaseModel):
    ...
    def forward(self, flabel, image, ds_image, infer=False):
        ...
        return [[loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_G_DIS, loss_G_SSIM, loss_D_real, loss_D_fake],
                None if not infer else real_image,
                None if not infer else input_flabel,
                None if not infer else fake_image_f,
                None if not infer else res,
                None if not infer else comp_image,
    ...
    def inference(self, real, flabel, ds):
        ...
        if self.opt.comp_type == 'compG': # use compG
            ...
            comp_image = self.compG.forward(compG_input)
            print("--- CompNet: %.4f seconds ---" % (time.time() - start_time))
        ...
        print("--- RecNet: %.4f seconds ---" % (time.time() - start_time))
